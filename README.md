<div align="center">

# 📝 Baccalauréat_game (Pays-Villes)

### Le jeu de lettres et de culture générale – Pays & Villes – en version mobile

[![Flutter](https://img.shields.io/badge/Flutter-3.x-02569B?logo=flutter&logoColor=white)](https://flutter.dev)
[![Dart](https://img.shields.io/badge/Dart-3.x-0175C2?logo=dart&logoColor=white)](https://dart.dev)
[![Firebase](https://img.shields.io/badge/Firebase-Config-FFCA28?logo=firebase&logoColor=black)](https://firebase.google.com)
[![License](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)

<br/>

[🎮 Jeu](#-présentation-du-jeu) • [📱 Aperçu](#-aperçu-de-lapplication) • [⚙️ Fonctionnalités](#️-fonctionnalités) • [🛠 Stack](#-stack-technique) • [🚀 Installation](#-installation) • [👨‍💻 Auteur](#-auteur)

<br/>

</div>

---

## 📋 À propos

**Baccalauréat_game (Pays-Villes)** est une application mobile de lettres et de culture générale inspirée du célèbre jeu du *Petit Bac*.  
Le projet a été développé avec **Flutter**, **pour le fun** et dans un **objectif pédagogique**, afin de pratiquer le développement mobile et d’appliquer les acquis précédemment appris.

---

## 🎮 Présentation du jeu

- Génération d’une **lettre aléatoire**
- Catégories **Pays, Villes, Prénoms, Objets**
- Saisie des réponses par le joueur
- **Calcul automatique des scores**

---

## 📱 Aperçu de l’application

### 🔐 Authentification

| Connexion | Inscription |
|:---:|:---:|
| <img src="screen_shots/cnx.jpeg" width="220"/> | <img src="screen_shots/signin.jpeg" width="220"/> |

---

### 🏠 Tableau de bord

| Dashboard 1 | Dashboard 2 | Dashboard 3 |
|:---:|:---:|:---:|
| <img src="screen_shots/dashboard1.jpeg" width="220"/> | <img src="screen_shots/dashboard2.jpeg.png" width="220"/> | <img src="screen_shots/dashboard3.jpeg" width="220"/> |

---

### 🎮 Jeu – Mode Solo

| Nouvelle partie 1 | Nouvelle partie 2 |
|:---:|:---:|
| <img src="screen_shots/newGameSolo1.jpeg" width="220"/> | <img src="screen_shots/newGameSolo2.jpeg" width="220"/> |

| Jeu en cours 1 | Jeu en cours 2 |
|:---:|:---:|
| <img src="screen_shots/gameSolo1.jpeg" width="220"/> | <img src="screen_shots/gameSolo2.jpeg" width="220"/> |

---

### 👥 Partie jointe / Multijoueur

| Rejoindre une partie |
|:---:|
| <img src="screen_shots/jointGame.jpeg" width="250"/> |

---

### 🧾 Résultats & Classement

| Résultats | Classement |
|:---:|:---:|
| <img src="screen_shots/result_screen.png" width="220"/> | <img src="screen_shots/classement.jpeg" width="220"/> |

---

### 📊 Historique des parties

| Historique |
|:---:|
| <img src="screen_shots/histo.jpeg" width="250"/> |

---

### ⚙️ Paramètres

| Paramètres |
|:---:|
| <img src="screen_shots/params.jpeg" width="250"/> |

---

## ⚙️ Fonctionnalités

- 🎲 Génération aléatoire des lettres
- 🌍 Catégories Pays & Villes
- 🧠 Logique de jeu automatisée
- 🧮 Calcul des scores
- 🎨 Interface moderne
- 📱 Android, iOS et Web

---

## 🛠 Stack technique

| Technologie | Rôle |
|------------|------|
| Flutter 3.x | UI cross-platform |
| Dart | Langage principal |
| Firebase | Configuration |
| JSON Assets | Dictionnaires |

---

## 🏗 Architecture

```
lib/
├── main.dart
├── screens/
├── models/
├── services/
└── providers/
```

---

## 🚀 Installation

```bash
git clone https://github.com/faceoff2003/baccalaureat-flutter.git
cd baccalaureat-flutter
flutter pub get
flutter run
```

---

## 🎯 Objectif

Projet personnel réalisé pour :
- pratiquer Flutter
- consolider les acquis
- enrichir un portfolio

---

## 👨‍💻 Auteur

<div align="center">

**William Soulayman**

[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/faceoff2003)
[![Portfolio](https://img.shields.io/badge/Portfolio-FF5722?style=for-the-badge&logo=google-chrome&logoColor=white)](https://soulayman.be)

*Développeur Full Stack - Diplômé en Informatique de Gestion (EAFC Colfontaine, 2025)*

</div>

---

<div align="center">

### ⭐ Si ce projet vous a plu, n'hésitez pas à lui donner une étoile !

<br/>

Made with ❤️ and ☕ in Belgium 🇧🇪

</div>
